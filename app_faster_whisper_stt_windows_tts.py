...
def load_whisper_local(model_name: str = "base"):
    ...
    On GPU: we can use int8_float16 for best CPU/GPU speed.
    On CPU: fall back to int8 (or float32) since int8_float16 is unsupported.
    """
    device = "cuda" if torch.cuda.is_available() else "cpu"

    if device == "cuda":
        # On GPU, int8_float16 is usually supported and fastest.
        return WhisperModel(model_name, device=device, compute_type="int8_float16")
    else:
        # On CPU (Windows), use int8 or float32. int8 is much faster than float32.
...
def transcribe_audio(wav_input) -> str:
    """
    Accepts either:
      • the dict returned by mic_recorder, or
      • raw bytes of a .wav file.
    Uses Faster Whisper to transcribe and returns the combined text.
    """
    # 1) Extract raw bytes if needed
    ...
    # 2) Write to a temporary .wav file so Faster Whisper can read it
    ...
    try:
        # 3) Run transcription with a small beam size (you can tweak beam_size for quality/speed)
        segments, _info = model.transcribe(
            tmp_path,
            beam_size=5,          # smaller beam = faster; increase for higher quality
            suppress_blank=True
        )
        # 4) Stitch all segment texts together
...
if st.session_state.result:
    ...
    if st.button("🔊 Speak this quotation"):
        ans = _preprocess_for_tts(st.session_state.result["answer"])
        speak_async(ans)
