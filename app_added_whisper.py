...
# ────────────────────────────────────────────────────────────────────────────────
# 1) Imports for Faster Whisper STT and Windows TTS
# ────────────────────────────────────────────────────────────────────────────────
import torch
...
import tempfile
import threading
from tts import speaker  # Assumes you have a 'tts.py' that exposes `speaker.Speak(...)`
# ────────────────────────────────────────────────────────────────────────────────
...
# ────────────────────────────────────────────────────────────────────────────────
# 3) PowerPoint‐generation utility
# ────────────────────────────────────────────────────────────────────────────────
...
# ────────────────────────────────────────────────────────────────────────────────
# 5) Faster Whisper STT: model loader + transcription
# ────────────────────────────────────────────────────────────────────────────────
@st.cache_resource(show_spinner="Loading Faster Whisper model…")
def load_whisper_local(model_name: str = "base"):
    """
    Returns a cached Faster WhisperModel instance.
    On GPU: use int8_float16; on CPU: fallback to int8.
    """
    device = "cuda" if torch.cuda.is_available() else "cpu"
    if device == "cuda":
        return WhisperModel(model_name, device=device, compute_type="int8_float16")
    else:
        return WhisperModel(model_name, device=device, compute_type="int8")

def transcribe_audio(wav_input) -> str:
    """
    Accepts either the dict from mic_recorder or raw bytes.
    Uses Faster Whisper to transcribe, returns the full text.
    """
...
# ────────────────────────────────────────────────────────────────────────────────
# 6) Windows TTS: speak on a background thread
# ────────────────────────────────────────────────────────────────────────────────
...
def _preprocess_for_tts(raw: str) -> str:
    ...
    • Collapse multiple lines into sentences separated by a pause.
    """
    cleaned_lines = []
    for line in raw.splitlines():
        line = line.strip()
        # Keep heading wording but drop the hashes (e.g. "## Quotation 1" → "Quotation 1")
        if line.startswith("##"):
...
logo_base64 = get_base64_logo("otsuka_im.png")
...
for key in ["result", "feedback", "user_query", "active", "pdf_bytes", "slide_bytes", "text_query", "just_transcribed"]:
...
# ────────────────────────────────────────────────────────────────────────────────
# 9) MIC + TRANSCRIPTION + TEXT INPUT
# ────────────────────────────────────────────────────────────────────────────────
...
# 2) Mic recorder icon (centered below)
...
# Only transcribe once per spoken event
...
# Callback to reset flag when user types
def reset_transcription_flag():
    st.session_state.just_transcribed = False

# The text_input uses the same key "text_query" and resets the flag on edit
...
if st.session_state.result:
    ...
    # ────────────────────────────────────────────────────────────────────────────
    # 10) Windows TTS: Speak the quotation aloud
    # ────────────────────────────────────────────────────────────────────────────
    if st.button("🔊 Speak this quotation"):
        answer_text = _preprocess_for_tts(st.session_state.result["answer"])
        speak_async(answer_text)
    # ────────────────────────────────────────────────────────────────────────────

    # ----------------- Feedback -------------------
    if st.session_state.active:
        st.markdown("---")
        st.subheader("Any changes or feedback you would like to recommend?")
